...
init() {
	...
	if [ ! -f ${GENEWEB_HOME}/etc/gwsetup_only ]; then
		gwlaunch_log "Database ${GENEWEB_HOME}/etc/gwsetup_only not found, creating it..."
		echo echo $(hostname -I | cut -d"." -f1,2,3).1 >> ${GENEWEB_HOME}/etc/gwsetup_only
	else
		gwlaunch_log "Database ${GENEWEB_HOME}/etc/gwsetup_only exists."
	fi
